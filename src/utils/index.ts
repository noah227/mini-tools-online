...
import {TMetaContent} from "@/types/common";
...
export const withMetaContent = <K extends keyof TMetaContent>(route?: RouteLocationNormalizedLoaded) => {
    route = route || useRoute()
    const meta = route.meta
    const content = meta.content
    return computed(() => {
        return {...content} as Required<Pick<TMetaContent, K>>
    })
}
